...
module ADD.Scavenger.Algebra2.ChallengeSpec where

import ADD.Scavenger.Algebra2.Challenge (Challenge, Commutative, Results (..), Results, ValidChallenge, andThen, both, bottom, clue, eitherC, empty, gate, reward, runChallenge)
import ADD.Scavenger.Algebra2.ClueState (ClueState)
import ADD.Scavenger.Algebra2.InputFilter (CustomFilter, InputFilter)
import ADD.Scavenger.Algebra2.InputFilterSpec (Test (..))
...
spec :: Spec
spec = modifyMaxSuccess (const 100) $ traverse_ (uncurry prop) quickspec_laws
