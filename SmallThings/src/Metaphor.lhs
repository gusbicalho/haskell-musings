...
Also, I apologize upfront for the silly examples.)

> You can use [markdown-unlit](https://github.com/sol/markdown-unlit) to load
> this file as a Literate Haskell file. If you don't want to install it
> globally on your system, something like this should work:
>
> ```bash
...
We tie together two ideas (the types `s` and `u`) such that we can use data from
on of them to influence the other. On the `oneHand`, we use a `u` to modify an
`s` into a `t`. On the `otherHand`, we can also use an `s` to modify a `u` into
a `v`.
...
If we just look at the types, `Metaphor s u t v ~ Coin (s, u) t v`.
...
[this blog post](http://comonad.com/reader/2012/mirrored-lenses/):
